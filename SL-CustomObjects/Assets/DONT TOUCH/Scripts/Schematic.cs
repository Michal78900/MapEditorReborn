...
public class Schematic : SchematicBlock
{
    ...
    public void CompileSchematic()
    {
        ...
        if (TryGetComponent(out Rigidbody rigidbody))
            rigidbodyDictionary.Add(rootObjectId,
                new SerializableRigidbody(rigidbody.isKinematic, rigidbody.useGravity, rigidbody.constraints,
                    rigidbody.mass));
        ...
        foreach (Transform obj in GetComponentsInChildren<Transform>())
        {
                    ...
                    case BlockType.Teleport:
                    {
                        if (obj.TryGetComponent(out TeleportComponent teleport))
                        {
                            ...
                            for (int i = 0; i < teleport.TargetTeleporters.Count; i++)
                            {
                                ...
                                teleport.TargetTeleporters[i].Id =
                                    teleport.TargetTeleporters[i].Teleporter.transform.GetInstanceID();
            ...
            if (obj.TryGetComponent(out Animator animator) && animator.runtimeAnimatorController != null)
            {
                ...
                BuildPipeline.BuildAssetBundle(animator.runtimeAnimatorController,
                    animator.runtimeAnimatorController.animationClips,
                    Path.Combine(schematicDirectoryPath, animator.runtimeAnimatorController.name),
                    AssetBundleBuildOptions, EditorUserBuildSettings.activeBuildTarget);
            }

            if (obj.TryGetComponent(out rigidbody))
                rigidbodyDictionary.Add(objectId,
                    new SerializableRigidbody(rigidbody.isKinematic, rigidbody.useGravity, rigidbody.constraints,
                        rigidbody.mass));
        ...
        if (teleporters.Count > 0)
            File.WriteAllText(Path.Combine(schematicDirectoryPath, $"{name}-Teleports.json"),
                JsonConvert.SerializeObject(teleporters, Formatting.Indented,
                    new JsonSerializerSettings() { NullValueHandling = NullValueHandling.Ignore }));
