...
public static class ObjectExtensions
{
    ...
    public static GameObject GetObjectByMode(this ObjectType toolGunMode) => ObjectPrefabs.TryGetValue(toolGunMode, out var obj) ? obj : null;
    ...
    {
        var message = "<size=30>Selected object type: <color=yellow><b>{objectType}</b></color></size>\n";

        if (mapObject is not RoomLightObject)
        {
            var relativePosition = mapObject.RelativePosition;
            var relativeRotation = mapObject.RelativeRotation;
        ...
        player.ReceiveHint(message, 9999f);
    }
    ...
    {
        var indicator = mapObject.AttachedIndicator;
