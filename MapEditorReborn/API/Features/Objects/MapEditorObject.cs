...
public abstract class MapEditorObject : MonoBehaviour
{
    ...
    /// <summary>
    /// Gets or sets forced <see cref="API.Enums.RoomType"/> of the object.
    /// </summary>
    ...
    public Vector3 RelativeRotation
    {
        get
        {
            ...
            var rotation = CurrentRoom.Type == RoomType.Surface ? transform.eulerAngles : transform.eulerAngles - CurrentRoom.transform.eulerAngles;
    ...
    public Room FindRoom()
    {
        ...
        Room room = Room.Get(gameObject.transform.position);
    ...
    public static Color GetColorFromString(string colorText)
    {
        ...
        var charTab = colorText.Split(new[] { ':' });

        if (charTab.Length >= 4)
        {
            if (float.TryParse(charTab[0], out var red))
                color.r = red / 255f;

            if (float.TryParse(charTab[1], out var green))
                color.g = green / 255f;

            if (float.TryParse(charTab[2], out var blue))
                color.b = blue / 255f;

            if (float.TryParse(charTab[3], out var alpha))
                color.a = alpha;
