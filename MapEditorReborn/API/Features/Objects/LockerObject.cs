...
public class LockerObject : MapEditorObject
{
    ...
    private void HandleItems()
    {
        ...
        if (Base.ShuffleChambers)
        {
            ...
            var chambersRandomValues = Base.Chambers.Values.OrderBy(x => Random.value).ToList();
            for (var i = 0; i < Base.Chambers.Count; i++)
            {
        ...
        for (var i = 0; i < Locker.Chambers.Length; i++)
        {
            ...
            var chosenLoot = Choose(Base.ShuffleChambers ? chambersCopy[i] : Base.Chambers[i]);
            if (chosenLoot == null)
                continue;
