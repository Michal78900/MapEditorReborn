...
public class TeleportObject : MapEditorObject
{
    ...
    private static int Choose(List<TargetTeleporter> teleports)
    {
        ...
        foreach (var elem in teleports.ToList())
        {
            if (elem.Chance <= 0f)
            {
    ...
    public TeleportObject Init(SerializableTeleport serializableTeleport, bool first = false)
    {
        ...
        if (first)
        {
            ...
            var lastTeleport = API.SpawnedObjects.LastOrDefault(x => x is TeleportObject teleport) as TeleportObject;
    ...
    public override void UpdateObject() => RefreshTargets();
    ...
    public void RefreshTargets()
    {
        Timing.CallDelayed(0.1f, () =>
        {
            ...
            try
            {
                foreach (var mapEditorObject in API.SpawnedObjects)
                {
                    ...
                    {
                        foreach (var target in Base.TargetTeleporters)
                        {
                            if (schematic.ObjectFromId.TryGetValue(target.Id, out var transform) && !TargetFromId.ContainsKey(target.Id))
    ...
    {
        if (!CanBeTeleported(collider, out var gameObject))
            return;

        Player player = Player.Get(gameObject);
        if (player is not null && !Base.AllowedRoles.Contains(player.Role))
            return;

        var choosenTeleporter = Choose(Base.TargetTeleporters);
        if (choosenTeleporter == -1)
            return;

        var target = TargetFromId[choosenTeleporter];
        ...
        var teleportSoundId = ev.TeleportSoundId;
    ...
    private bool CanBeTeleported(Collider collider, out GameObject gameObject)
    {
        ...
        var flag =
            (!DecontaminationController.Singleton.IsDecontaminating || !Base.LockOnEvent.HasFlagFast(LockOnEvent.LightDecontaminated)) &&
    ...
    private void OnDestroy()
    {
        Timing.CallDelayed(0.1f, () =>
        {
            try
            {
                foreach (var teleport in TargetFromId.Values)
                {
