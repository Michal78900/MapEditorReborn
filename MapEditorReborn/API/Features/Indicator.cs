...
public static class Indicator
{
    ...
    {
        ItemType parsedItem = (ItemType)Enum.Parse(typeof(ItemType), itemSpawnPoint.Base.Item, true);
        ...
        var scale = parsedItem.IsWeapon() ? new Vector3(0.25f, 0.25f, 0.25f) : Vector3.one;

        var pickup = Item.Create(parsedItem).CreatePickup(itemSpawnPoint.transform.position + (Vector3.up * 0.1f * scale.y), Quaternion.identity, scale);
        pickup.IsLocked = true;

        var pickupGameObject = pickup.Base.gameObject;
    ...
    public static void SpawnObjectIndicator(PlayerSpawnPointObject playerSpawnPoint, IndicatorObject indicator = null)
    {
        ...
        var position = playerSpawnPoint.transform.position;

        var dummyObject = Object.Instantiate(LiteNetLib4MirrorNetworkManager.singleton.playerPrefab);
        ...
        //     ccm.cur = RoleType.Tutorial;
        //     ccm.GodMode = true;
        // }

        var dummyNickname = playerSpawnPoint.Base.SpawnableTeam.ToString();
    ...
    public static void SpawnObjectIndicator(RagdollSpawnPointObject ragdollSpawnPoint, IndicatorObject indicator = null)
    {
        ...
        var position = ragdollSpawnPoint.transform.position;
        ...
        var roleType = ragdollSpawnPoint.Base.RoleType;
    ...
    public static void SpawnObjectIndicator(LightSourceObject lightSource, IndicatorObject indicator = null)
    {
        ...
        var pickup = Item.Create(ItemType.SCP2176).CreatePickup(lightSource.transform.position, Quaternion.Euler(180f, 0f, 0f), Vector3.one * 2f);
        pickup.IsLocked = true;
