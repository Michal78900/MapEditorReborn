...
public class Modify : ICommand
{
    ...
    {
        get => "Allows modifying properties of the selected object.";
    }
    ...
    public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
    {
        ...
        var instance = mapObject.GetType().GetField("Base").GetValue(mapObject);
        var properties = instance.GetType().GetProperties().Where(x => (Type.GetTypeCode(x.PropertyType) != TypeCode.Object || x.PropertyType == typeof(float?)) && !x.Name.Contains("RoomType")).ToList();
        ...
        var foundProperty = properties.FirstOrDefault(x => x.Name.ToLower().Contains(arguments.At(0).ToLower()));
        ...
        try
        {
            ...
            {
                var spacedString = arguments.At(1);
    ...
    private bool HandleTeleport(TeleportObject teleport, ArraySegment<string> arguments, Player player, ref string response)
    {
        if (arguments.Count == 0)
        {
            ...
            foreach (var targetTeleporter in teleport.Base.TargetTeleporters)
            {
        ...
        switch (arguments.At(0).ToLower())
        {
            case "add":
            {
                if (!int.TryParse(arguments.At(1), out var id) || id < 0)
                {
            ...
            default:
            {
                if (!int.TryParse(arguments.At(0), out var id) || id < 0)
                {
                    response = "You need to provide a valid teleport id!";
                    return false;
                }
                ...
                if (!arguments.At(2).TryParseToFloat(out var chance) || chance <= 0)
                {
