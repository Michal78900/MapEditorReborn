...
public class List : ICommand
{
    ...
    {
        get => "Shows the list of all available maps.";
    }
    ...
    public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
    {
        ...
        var builder = NorthwoodLib.Pools.StringBuilderPool.Shared.Rent();
        ...
        if (arguments.Count == 0)
        {
            ...
            foreach (var directoryPath in Directory.GetDirectories(MapEditorReborn.SchematicsDir))
            {
                var jsonFilePath = Directory.GetFiles(directoryPath).FirstOrDefault(x => x.EndsWith(".json"));
        ...
        var paths = fileNames.Where(x => x.Contains(arguments.At(0))).ToArray();
        ...
        foreach (var path in paths)
        {
            ...
            {
                var data = Utf8Json.JsonSerializer.Deserialize<SchematicObjectDataList>(File.ReadAllText(Path.Combine(path, Path.GetFileNameWithoutExtension(path) + ".json")));
