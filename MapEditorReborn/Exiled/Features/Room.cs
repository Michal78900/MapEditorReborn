...
public class Room : MonoBehaviour, IWorldSpace
{
    ...
    {
        get => RoomIdentifierToRoom.Values;
    }
    ...
    {
        get => Player.GetPlayers().Where(player => player.IsAlive && player.Room is not null && (player.Room.transform == Transform));
    }
    ...
    public static Room Get(RoomIdentifier roomIdentifier) => RoomIdentifierToRoom.TryGetValue(roomIdentifier, out var room)
        ? room
        : null;
    ...
    public static Room Get(Vector3 position) => RoomIdUtils.RoomAtPositionRaycasts(position, false) is { } identifier ? Get(identifier) : null;
    ...
    {
        var rooms = zoneType is not ZoneType.Unspecified ? Get(r => r.Zone.HasFlag(zoneType)) : List;
    ...
    private void Awake()
    {
        ...
        //    TeslaGate = TeslaGateController.Singleton.TeslaGates.Single(x => this == x.Room);
    }
}